...
<head>
    ...
    <title>Custom Salary Slip - {{ $payroll->empCode }}</title>
    <style>
        @page {
            margin: 15px;
        }
...
<body>
    <div class="container">
        ...
        <div class="company-info">
            <div class="company-name">{{ $company->companyName ?? 'MACO CORPORATION INDIA PVT LTD' }}</div>
            <div>{{ $company->addressLine1 ?? 'Sukh Sagar Building, Suite No. 7A, 7th Floor, 2/5 Sarat' }}</div>
            <div>{{ $company->addressLine2 ?? 'Bose Road,, Kolkata,' }}</div>
            <div>{{ $company->city ?? 'West Bengal' }}-{{ $company->pincode ?? '700020' }}</div>
        </div>
        <div class="header">
